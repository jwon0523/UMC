...
class HomeCollectionLayout {
  ...
  private static func createBannerSection() -> NSCollectionLayoutSection {
    ...
    /// 섹션의 스크롤 방향
    /// - continuous: 부드러운 가로 스크롤 동작
    /// - groupPaging: 그룹 단위로 페이지 스크롤
    /// - groupPagingCentered: groupPaging과 비슷하지만, 페이지가 항상 중앙 정렬됨
    /// - continuousGroupLeadingBoundary: 콘텐츠가 스크롤 될 때 그룹의 첫 번째 요소가 항상 화면의 왼쪽 경계에 정렬됨
  ...
  private static func createHeaderItem() -> NSCollectionLayoutBoundarySupplementaryItem {
    ...
    header.pinToVisibleBounds = true // 화면에 고정
