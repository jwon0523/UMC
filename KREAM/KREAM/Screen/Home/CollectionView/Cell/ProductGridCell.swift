...
class ProductGridCell: BaseCollectionCell {
  ...
  /// 상품 구매 수 라벨
  private lazy var purchaseCount: UILabel = makeLabel(
    font: UIFont.systemFont(ofSize: 10, weight: .regular),
    UIColor(red: 0.251, green: 0.272, blue: 0.294, alpha: 1)
  )
  ...
  override func configure(model: any RequiredCellProtocol) {
    ...
    self.imageView.image = UIImage(
      named: productGridModel.imageView
    )?.downSample(scale: 0.3)
    self.purchaseCount.text = productGridModel.purchaseCnt
    self.tagButton.setImage(
      setTagButtonImage(isMark: productGridModel.selectedTag),
      for: .normal
    )
